...
echo "== tests =="
"$PY" "$ROOT/tests/dual_branch/test_dual_branch.py" > /dev/null && echo "9/9 passed"
echo "== status =="
cat "$ROOT/runs/dual_branch/status.txt"
echo "== gate =="
"$PY" -c "import json; print(json.dumps(json.load(open('$ROOT/runs/dual_branch/dual_branch_gate.json')), indent=1))"
echo "== summary =="
"$PY" - <<'PY'
import csv
with open("/data1/LWR/vranlee/SERVER_ONLY/avis/OCD_OVMOT/outputs/dual_branch/metrics/final_summary.csv") as f:
