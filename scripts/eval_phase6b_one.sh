#!/usr/bin/env bash
# Evaluate one Phase 6B DSCT model on the frozen Q1 stream:
# OVTR dsct eval -> proposals CSV -> strict-causal jointcsv -> physical eval.
set -euo pipefail
...
OUT=$ROOT/outputs/iclr27_phase6b/q1/$NAME
...
CUDA_VISIBLE_DEVICES="$GPU" PYTHONPATH=. "$OVTR_PY" eval.py \
  ...
  --score_mode dsct --num_workers 4 --sampler_lengths 2 \
  "${VIDEO_ARGS[@]}" \
...
if [[ "${SKIP_STRICT:-0}" != "1" ]]; then
  ...
  if (( NROWS <= 0 )); then
    mkdir -p "$ROOT/outputs/iclr27_phase6b/strict_eval/${NAME}_dsct"
    cat > "$ROOT/outputs/iclr27_phase6b/strict_eval/${NAME}_dsct/summary.json" <<EOF
...
EOF
    ...
    PYTHONPATH="$ROOT" /home/lwr/anaconda3/envs/locatemot/bin/python \
      ...
      --device "cuda:$GPU" \
      --out "$ROOT/outputs/iclr27_phase6b/strict_eval/${NAME}_dsct" \
      > "$OUT/strict.log" 2>&1
  fi
else
...
PYTHONPATH="$ROOT" python3 "$ROOT/src/iclr27_phase6a/evaluation/physical_eval.py" \
  ...
  --out "$ROOT/outputs/iclr27_phase6b/physical_eval/${NAME}.json" \
  > "$OUT/physical.log" 2>&1

PYTHONPATH="$ROOT" python3 "$ROOT/src/iclr27_phase6a/tests/causal_contract_tests.py" \
  --csv "$OUT/proposals_dev.csv" \
  --out "$ROOT/outputs/iclr27_phase6b/strict_eval/${NAME}_causal_contract.json" \
...
PYTHONPATH="$ROOT" python3 "$ROOT/src/iclr27_phase6a/evaluation/objectness_audit.py" \
  ...
  --out "$ROOT/outputs/iclr27_phase6b/strict_eval/${NAME}_objectness_audit.json" \
