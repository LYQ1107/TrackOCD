 ...
 for fold in range(4):
  ...
  for p in PREFIXES:
   ...
   for r in val:
    ...
    with torch.no_grad(): z,a,_=bridge(torch.tensor(q).view(1,-1),torch.tensor(ctx).view(1,-1),torch.tensor([float(raw.max())]),torch.tensor([sq]).view(1),True); bs=z.numpy()[0]@c.T; rmarg=torch.tensor([float(raw[:len(sk)].max()-raw[len(sk):].max())]); bmarg=torch.tensor([float(bs[:len(sk)].max()-bs[len(sk):].max())]); pr=gate(rmarg,bmarg,torch.tensor([sq]),a,torch.zeros(1)).item()
    chosen=bs if pr>=0.5 else raw; x,y,g=calc(raw,lab); r1.append(x);rm.append(y);rg.append(g); x,y,g=calc(chosen,lab); b1.append(x);bm.append(y);bg.append(g); use.append(pr>=0.5)
   per[str(p)]={'raw':{'r1':float(np.mean(r1)) if r1 else 0,'map':float(np.mean(rm)) if rm else 0,'hard_gap':float(np.mean(rg)) if rg else 0},'trained_selective':{'r1':float(np.mean(b1)) if b1 else 0,'map':float(np.mean(bm)) if bm else 0,'hard_gap':float(np.mean(bg)) if bg else 0},'bridge_use_rate':float(np.mean(use)) if use else 0}
  folds.append({'fold':fold,'prefix':per})
 agg={str(p):{n:{m:float(np.mean([f['prefix'][str(p)][n][m] for f in folds])) for m in ('r1','map','hard_gap')} for n in ('raw','trained_selective')} for p in PREFIXES}; atomic(OUT/'metrics/trained_selective_replay.json',{'protocol':'phase42_trained_selective_gate','folds':folds,'aggregate':agg,'sealed_inputs_not_read':['DEV+','Q1','public labels','future','IDs/text/held GT']}); atomic(OUT/'completion/retrieval.done',{'metrics':str(OUT/'metrics/trained_selective_replay.json'),'p16':agg['16']}); print(json.dumps({'p16':agg['16']},indent=2))
