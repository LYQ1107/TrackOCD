...
run_one() {
  local fold="$1"
  local source="$2"
  local tag="h1_false_merge_reset_resume2_f${fold}"
  if [[ -f "$OUT/completion/${tag}.done" ]]; then echo "SKIP_DONE $tag"; return 0; fi
...
run_one 2 "$CKPT/h1_false_merge_reset_resume1_f2_step028000.pt"
run_one 3 "$CKPT/c0v2_fix2_formal_f3.pt"
echo "H1_RESUME2_FORMAL_COMPLETE"
