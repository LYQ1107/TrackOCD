...
def main()->None:
    ...
    for fold in (int(v) for v in args.folds.split(",") if v.strip()):
        ...
        for step in range(1,args.steps+1):
            pool=[g for g in fit if (g in relset)==(rng.random()<.5)] if relset and non else fit; gi=int(pool[rng.integers(0,len(pool))]); weight=2.0 if gi in relset else 1.0; inds=flat[offsets[gi]:offsets[gi+1]];losses.append(model.step(xf[inds],int(targets[gi]),weight));
