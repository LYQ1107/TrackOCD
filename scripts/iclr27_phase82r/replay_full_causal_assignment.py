...
def main() -> None:
    ...
    for video in videos:
        ...
        for key in frames:
            ...
            iw, ih = float(size[0]), float(size[1]); decisions: dict[int, dict[str, Any]] = {}
            ...
            for tid, (idx, row, box) in best_rows.items():
                ...
                states[tid] = {"track": tid, "step": step, "box": box, "app": feats[idx].copy(), "velocity": vel}
        stats["videos"] += 1
    out_path = ROOT / "outputs/iclr27_phase82r/replays" / f"{args.tag}.jsonl"; atomic_jsonl(out_path, out_rows)
    summary = {"schema_version": "trackocd.phase82r.full_causal_assignment_replay.v1", "created_utc": dt.datetime.now(dt.timezone.utc).isoformat(), "native_path": str(NATIVE), "native_sha256": sha256(NATIVE), "appearance_path": str(APPEARANCE), "appearance_sha256": sha256(APPEARANCE), "tag": args.tag, "accept_score": 0.5, "videos": len(videos), "rows": len(out_rows), "stats": dict(stats), "output": str(out_path), "observed_step_map": True, "q0_non_birth_proposal_preserved": True, "public_dev_q1_sealed_accessed": False, "future_rows_or_tracks": False, "ids_as_model_input": False}
