...
def run_stream(ckpt:Path, device:str, max_videos=None, use_motion: bool = False, max_miss: int = 8, use_appearance: bool = False, geometry: bool = False):
    import torch
    ...
    if geometry:
        runtime=CausalGeometryRuntime(max_miss=int(max_miss), max_tracks=512)
    else:
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--checkpoint',required=False,default=''); ap.add_argument('--device',default='cuda:0'); ap.add_argument('--tag',default='formal'); ap.add_argument('--max-videos',type=int); ap.add_argument('--motion',action='store_true'); ap.add_argument('--appearance',action='store_true'); ap.add_argument('--geometry',action='store_true'); ap.add_argument('--max-miss',type=int,default=8); args=ap.parse_args()
    import torch
    ck=Path(args.checkpoint) if args.checkpoint else Path('/dev/null'); events=load_event_records(); stream=run_stream(ck,args.device,args.max_videos,use_motion=args.motion,max_miss=args.max_miss,use_appearance=args.appearance,geometry=args.geometry); out=event_metrics(stream,events); result={'schema_version':'phase81p.replay_metrics.v1','created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'checkpoint':str(ck.resolve()) if not args.geometry else None,'checkpoint_sha256':hashlib.sha256(ck.read_bytes()).hexdigest() if not args.geometry else None,'event_observability_input':str(EVENT_OBS),'event_observability_sha256':hashlib.sha256(EVENT_OBS.read_bytes()).hexdigest(),'aggregate':out['aggregate'],'events':out['events'],'protocol':{'positive_denominator':76,'negative_denominator':76,'prefixes':[1,2,4,8,16],'labels_joined_before_inference':False,'future_rows_or_tracks':False,'ids_as_model_input':False,'causal_motion_prediction':bool(args.motion),'causal_appearance_descriptor':bool(args.appearance),'geometry_only':bool(args.geometry),'max_miss':int(args.max_miss)}}
