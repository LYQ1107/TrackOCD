 ...
 for fold in range(4):
  ...
  for p in PREFIXES:
   ...
   for r in val:
    ...
    chosen=bs if pr>=0.5 else raw; x,y,g=calc(raw,lab); rr.append(x);rm.append(y);rg.append(g); x,y,g=calc(chosen,lab); sr.append(x);sm.append(y);sg.append(g); use.append(pr>=0.5);agree.append(bool((pr>=0.5)==bool(sq>=0.2 and float(bmarg)>=float(rmarg)+0.005)))
   per[str(p)]={'raw':{'r1':float(np.mean(rr)) if rr else 0,'map':float(np.mean(rm)) if rm else 0,'hard_gap':float(np.mean(rg)) if rg else 0},'learned':{'r1':float(np.mean(sr)) if sr else 0,'map':float(np.mean(sm)) if sm else 0,'hard_gap':float(np.mean(sg)) if sg else 0},'bridge_use_rate':float(np.mean(use)) if use else 0,'teacher_agreement':float(np.mean(agree)) if agree else 0}
  folds.append({'fold':fold,'prefix':per})
 agg={str(p):{n:{m:float(np.mean([f['prefix'][str(p)][n][m] for f in folds])) for m in ('r1','map','hard_gap')} for n in ('raw','learned')} for p in PREFIXES}; atomic(OUT/'metrics/policy_gate_retrieval.json',{'protocol':'phase43_policy_distilled_selective_gate','folds':folds,'aggregate':agg,'sealed_inputs_not_read':['DEV+','Q1','public labels','future','IDs/text/held GT']}); atomic(OUT/'completion/retrieval.done',{'metrics':str(OUT/'metrics/policy_gate_retrieval.json'),'p16':agg['16']}); print(json.dumps({'p16':agg['16']},indent=2))
