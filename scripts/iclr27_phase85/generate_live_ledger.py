...
def expected_provenance() -> list[dict[str, Any]]:
    return [
        ...
        {"section_name": "support_raw_defer", "expected_route": "raw source-mean top32; TRAIN defer head only (p>=0.5 -> DEFER); reranker output ignored", "expected_tag": None, "path": METRICS / "support_event_replay_raw_defer_v1.json"},
        ...
        {"section_name": "leakage_contract", "expected_route": None, "expected_tag": None, "path": AUDIT / "leakage_contract.json"},
...
def main() -> None:
    ...
    ledger = {
        "schema_version": "trackocd.phase85.research_ledger.v1", "generated_utc": dt.datetime.now(dt.timezone.utc).isoformat(), "registration": reg, "finalization_lock": lock, "git_head": git("rev-parse", "HEAD"), "git_status": git("status", "--porcelain"), "resources": resources(), "next_action": "finalize only inside the registered deadline-minus-45-minute interval; all registered physical/support routes are complete and no controller/alignment/sealed route is authorized", "remaining_seconds_to_deadline": max(0.0, (dt.datetime.fromisoformat(str(reg.get("deadline_utc")).replace("Z", "+00:00")) - dt.datetime.now(dt.timezone.utc)).total_seconds()), "stages": [
            ...
            {"stage": "B85S_raw_defer", "status": "SAFETY_FAIL", "artifact": artifact(METRICS / "support_event_replay_raw_defer_v1.json"), "p16": [x for x in load(METRICS / "support_event_replay_raw_defer_v1.json").get("summary", []) if x.get("prefix") == 16], "next_action": "retain negative support evidence"},
