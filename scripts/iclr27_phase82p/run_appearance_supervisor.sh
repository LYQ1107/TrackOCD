...
for shard in 0 1 2 3; do
  out="$SHARDS/${TAG}_shard_$(printf '%02d' "$shard").npz"
  done_marker="$OUT/completion/appearance_${TAG}_shard_${shard}.done"
  launched="$OUT/completion/appearance_${TAG}_shard_${shard}.launched"
  ...
  CUDA_VISIBLE_DEVICES="${GPUS[$shard]}" "$PY" "$ROOT/scripts/iclr27_phase82p/extract_q0_appearance.py" \
    ...
    > "$LOGS/${TAG}_shard_${shard}.log" 2>&1 &
  pids+=("$!"); names+=("$shard")
done
status=0
for i in "${!pids[@]}"; do
  if wait "${pids[$i]}"; then
    ...
    tmp="$OUT/completion/appearance_${TAG}_shard_${shard}.done.$$.tmp"
    printf 'complete shard=%s utc=%s output=%s\n' "$shard" "$(date -u +%FT%TZ)" "$out" > "$tmp"
    mv "$tmp" "$OUT/completion/appearance_${TAG}_shard_${shard}.done"
  else
