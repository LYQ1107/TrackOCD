#!/usr/bin/env python3
"""Extract the same DINOv2 crop descriptor for native Q0 event rows."""
...
def crop_box(image: Any, box: list[float], context: float = 0.10) -> Any:
    ...
    xa, ya = max(0.0, cx - bw * (1 + 2 * context) * 0.5), max(0.0, cy - bh * (1 + 2 * context) * 0.5)
    xb, yb = min(float(w), cx + bw * (1 + 2 * context) * 0.5), min(float(h), cy + bh * (1 + 2 * context) * 0.5)
...
def main() -> None:
    ...
    ap.add_argument("--device", default="cuda:0"); ap.add_argument("--batch", type=int, default=32); ap.add_argument("--max-rows", type=int, default=0); ap.add_argument("--out", type=Path, required=True)
    args = ap.parse_args()
    ...
    if args.out.exists() and Path(str(args.out) + ".json").exists():
    ...
    tensors: list[Any] = []; positions: list[int] = []; failures: list[dict[str, Any]] = []
    def flush() -> None:
        nonlocal tensors, positions
        if not tensors: return
        x = torch.cat(tensors, dim=0).to(args.device)
        with torch.no_grad():
            y = model.forward_features(x)["x_norm_clstoken"]
    ...
    meta = {"schema_version": "trackocd.phase82p.native_dinov2_appearance_shard.v1", "created_utc": dt.datetime.now(dt.timezone.utc).isoformat(), "shard": args.shard, "num_shards": args.num_shards, "rows": len(selected), "native_rows": len(rows), "native_sha256": sha256(NATIVE), "frames_root": str(FRAMES), "model_repo": str(HUB), "model": "facebookresearch/dinov2:dinov2_vitb14", "feature": "normalized x_norm_clstoken of current native Q0 crop", "dtype": "float16", "device": args.device, "batch": args.batch, "future_frames_used": False, "category_or_id_feature": False, "failures": failures, "out_sha256": sha256(args.out)}
    atomic_json(Path(str(args.out) + ".json"), meta); print(json.dumps(meta, indent=2, sort_keys=True))
