...
- B4 native matcher smoke (100) and fold0 targeted (500) completed with finite outputs and atomic checkpoints. Fold0 validation top-1/top-20 candidate recall was `0.3831/0.9175` at smoke and `0.4061/0.9060` targeted; targeted DEFER recall was `0.7689`. In p16 replay, targeted native-set support was selected on only 1/76 positive (0/76 both-side reliable) and 3/76 negative events. This early result is not a gate; the pre-registered four-fold formal run is still required.
- B4 native-set formal training completed all four folds at 1,000 updates without OOM. Validation top-1 recall was `[0.3946,0.1534,0.3036,0.0732]` and p16 replay selected support on 32/76 positives and 30/76 negatives, but only 4/76 positive events were both-side reliable (frozen Q0=25/76) and no negative event was reliable. This closes B4 as a learned native-set failure; a deterministic native-score/coverage audit is the remaining short high-value diagnostic before the fixed window closes.
...
- B4 deterministic native-score audit completed: native pool oracle p16 remains source/target/both `72/76,64/76,61/76`, but causal per-frame highest base-score selection reaches only `29/76,18/76,12/76` on positives and activates reliably on 6/76 negatives. This confirms a native candidate-source/ranking gap rather than an absent pool; no learned or deterministic selector has yet improved the frozen strict-O 25/76 reference.
- With native candidate coverage established but candidate-only rankers failing, the next short hypothesis is B5 cross-video support matching: use the completed source-track causal DINO vector to rank native target-frame candidates by appearance cosine. This is a post-hoc diagnostic first (no training, no controller); positive/negative event labels are used only to quantify selected native IoU, and no threshold/denominator is changed.
- B5 cross-video source-support diagnostic completed on the full 76+76 event manifest. Prior completed source-track Q0 appearance versus native target candidates produced p16 target reliable `20/76` and source+target `17/76`, below frozen Q0 target `40/76`/both `25/76`; negative target reliable activation was `8/76`. This closes the source-appearance-only hypothesis without controller or sealed access. Remaining time is reserved for final ledger/report generation after the original deadline-45-minute lock boundary.

## Phase83 resumed-window finalization guard (2026-09-04)

- Added and pushed the lock-aware `scripts/iclr27_phase83/generate_resumed_report.py` (`82d1d22`). It writes an atomic, data-driven `outputs/iclr27_phase83/audit/resumed_research_ledger.json` but refuses to create the final report while the original `2026-09-04T17:43:07Z` window is more than 45 minutes from its deadline. This preserves the original clock and prevents another premature closeout.
- The in-progress ledger records A2/A3 and O83/B2/B3/B4/B5 evidence, immutable 76+76 protocol, artifact hashes, markers, resource snapshot and symlink targets. At the first ledger capture (`2026-09-04T15:52:01Z`) finalization remained locked with 6,665 seconds to the deadline; no new training, controller, threshold, backbone or sealed/public evaluation was started.
- A read-only process-audit self-match in the generator was found (the generator command matched its own `iclr27_phase83` pattern); the filter was minimally corrected and pushed as `0987588`. A fresh ledger capture records an empty Phase83-process list; no task process was running and no scientific artifact changed.
- The original Phase83 window entered the authorized finalization interval at `2026-09-04T16:58:07Z`; the lock was released at `2026-09-04T16:58:28Z` without changing the registered deadline. The complete resumed report and decision were generated atomically from the frozen A2/A3/B2/B3/B4/B5 artifacts. Final status is `AUTONOMOUS_PHASE83_WINDOW_COMPLETE_WITH_NEGATIVE_EVIDENCE`: R83/A2/A3 and O83/B2/B3/B4/B5 failed or were unsafe, C83 and sealed/public evaluation stayed `NOT_RUN`, and no new model or controller work was started during finalization.

## Phase84 autonomous window (2026-09-04)

- Registered a new ten-hour Phase84 window at `2026-09-04T17:50:55Z` (deadline `2026-09-05T03:50:55Z`) in the independent `outputs/iclr27_phase84` namespace. Outputs target `/data2/usr_for_deadline/trackocd_phase84/project_outputs` through a project symlink; finalization lock is initially false. GPU0--3 were occupied by external jobs, while GPU4--9 were idle at registration; available RAM was about 96 GiB of 125 GiB. No process was started by Phase84 registration.
- Read-only Stage0 integrity audit found a report-source error: the frozen Phase83 A2 table uses the partial `physical_r_temporal.json` p16 values (`R@1=0.882735`, unsafe=5) rather than the actual A2 artifact `a2_temporal_r.json` (`R@1=0.880983`, raw `0.909890`, unsafe=8, 894 queries, 23,341/43,423 rows mapped). The two artifacts and report hashes are recorded in `outputs/iclr27_phase84/audit/phase83_a2_report_integrity.json`; no Phase83 inference was rerun or modified. `docs/iclr27_phase84/PHASE83_INTERFACE_CORRECTION.md` reclassifies A2 as incomplete native mapping evidence.
- A read-only physical-to-R callgraph was recorded in `physical_to_r_interface_callgraph.json`. A true Phase84 A branch must change canonical physical membership through causal unions over the full native Q0 stream before aggregating the frozen 984-query R universe; replacing vectors inside unchanged public track membership is insufficient. The frozen 0.8 CLS + 0.2 ROI 768-D feature, candidate rule, row-key and causal restrictions remain unchanged.

- Phase84 A84P smoke (`physical_smoke_r1`, two videos) failed before writing lineage because some native Q0 rows encode `candidate_rank=null`; direct integer conversion raised `TypeError`. This is a serialization-only root cause, not a scientific result and did not touch Phase83. The minimal parser repair maps null rank to deterministic `0`; the failed smoke remains recorded and will not be relabeled as complete.
- B84S 100-step smoke (`b84s_smoke_r1`, fold0) wrote only its `.launched` marker and failed before checkpoint/metrics: DEFER targets equal the candidate count, but the first implementation indexed the candidate gradient before checking that condition (`IndexError`). The marker is retained as failed evidence; the minimal repair uses an explicit `if target < candidate_count` branch, after which a fresh smoke tag is required.
- The first B84S formal supervisor launch (`b84s_formal_r1`) failed before any fold because the shell supervisor was accidentally invoked through the Python interpreter. Its aggregate `.launched` marker is retained without `.done` or fold artifacts; no scientific output was written. The correction is command-only (`bash scripts/iclr27_phase84/run_b84s_supervisor.sh`) and will use a fresh tag `b84s_formal_r2`.

## Phase84 continuation: completed physical replay and B84S formal training (2026-09-04)

- A84P true full-native temporal reassociation completed from the frozen Phase83 Q0 lineage and corrected native DINO cache. The output preserved all 682,335 rows across 370 videos and recorded 27,710 causal reconnect unions, 97,669 keeps, 2,281 lineage-collision groups and 15,799 same-frame collision fallbacks. The causal lineage SHA256 is `cb606267383782bada5d94320b4d91b459d708e27a25b08d87f5616a6a3405c6`; union-events SHA256 is `e384970c54102481e7e794c8127322a8a5e61dbad0eeee9fc39967fbd106b72b`. No task process remained after the single blocking supervisor.
- The Phase84 physical-to-R adapter passed Q0 raw parity (same 984-query denominator/order and zero vector error) and mapped 23,341/43,423 frozen public rows (1,194 prefix-16 tracks plus 104 explicit fallbacks). Canonical membership changed, but full TRAIN validation physical-R p16 was raw `R@1=0.893219/mAP=0.848374` versus physical `0.894135/0.871168`, hard-gap `0.189559/0.201346`, unsafe flips `6`; only 2/4 folds were non-decreasing in both metrics. A84P is therefore `A84P_TRUE_PHYSICAL_R_FAIL_WITH_VALID_CONTRACT`, not a controller result. Post-hoc TRAIN labels show 11,816 same-category versus 1,105 cross-category labeled unions and 92.16% of canonical roots contain multiple categories, so the fixed visual/geometry union is semantically contaminated.
- The registered same-space source-conditioned signal audit (corrected native DINO for both source and target) was strong enough to authorize B84S: p16 aggregate source-conditioned max-prototype `R@1=0.974369/mAP=0.910847/hard-gap=0.251195` versus raw `0.936698/0.879438/0.202140`, with 3 unsafe flips and 3/4 folds non-decreasing. This is a post-hoc TRAIN signal diagnostic only; labels are not model inputs.
- B84S manifest/cache completed from native Q0 candidate sets: 13,631 image groups, 464,146 candidate rows, 5,471 groups with a deterministic same-category/different-video TRAIN source and 7,554 explicit DEFER targets. The 19-D feature tensor contains only native candidate descriptors and source-conditioned cosine channels; GT/category/IDs/future/text are excluded from model input. Inputs and feature/cache hashes are recorded in `outputs/iclr27_phase84/manifests/b84s_native_manifest.json` and `source_track_native_cache.json`.
- B84S smoke `b84s_smoke_r1` retained a failed marker (DEFER gradient indexing); the minimal branch repair was pushed and fresh `b84s_smoke_r2` (100 updates) plus `b84s_targeted_r1` (500 updates) passed finite-logit/checkpoint checks. The first formal invocation `b84s_formal_r1` failed before any fold because the shell supervisor was sent to Python; its marker is retained as command-only evidence. Correct `bash` invocation `b84s_formal_r2` then completed all four 15-epoch folds on one bounded CPU worker with atomic checkpoints/markers and no OOM or external-process termination. Validation candidate top-1 recall was `[0.00174, 0.06019, 0.0, 0.02439]`, top-5 `[0.19053, 0.71456, 0.875, 0.21951]`; predicted DEFER dominated folds 0/1/2 and fold3 had poor DEFER recall. The formal learned selector is weak under the frozen disjoint validation distribution; its metrics are diagnostic only and no controller/StateMemory/threshold/sealed evaluation has been run.
- All Phase84 source/code changes through formal training were compiled where applicable and pushed to `origin/main`; old Phase83 artifacts were not modified. Public DEV+/Q1/new-model/sealed labels were not accessed. The next action is to aggregate the four formal metrics and perform one frozen event-level B84S selection replay; do not launch the optional alignment branch unless this learned selector materially improves selection over the registered B5 diagnostic.

## Phase84 B84S frozen replay and query-contract repair registration (2026-09-04)

- Frozen B84S formal aggregation is complete: validation weighted candidate top-1/top-5 recall `0.010623/0.283916`, candidate-or-DEFER accuracy `0.089412`, DEFER recall `0.882585`; the macro top-1 was `0.021581`. These values are from all four completed 15-epoch folds and are not held-event selection metrics.
- Native event replay evaluated all 760 records (76 positive + 76 negative events at each of five prefixes). At prefix16 B84S selected a candidate on 33/76 positives and 12/76 negatives, but only 9/76 positive selections reached IoU>=0.5 and 0/76 negatives did; same-space raw source-mean selection reached 20/76 positive and 8/76 negative reliable events. Positive selection did not improve over the registered B5 diagnostic, so no alignment branch or controller was started.
- Root-cause audit: `build_b84s_manifest.py` attached a single deterministic first same-category TRAIN source to each native image group. The trained selector consequently saw target groups, not the registered query/source pair distribution; event source tracks were never represented during training. This is an interface/data-contract failure, not evidence that corrected native source-conditioned visual signal is absent. A single minimal repair `B84S-Q` is preregistered in `docs/iclr27_phase84/PHASE84_B84S_QUERY_CONTRACT_REPAIR.md`: build groups from the existing Phase30 TRAIN-only query/support records, retain native action sets and the same 19-D linear listwise+DEFER model, cap two positive and two hard-DEFER groups per source, and exclude all event videos. No threshold/controller/StateMemory/backbone/public/sealed change is authorized.
- The first parameterized B84S-Q event replay attempt failed before writing any output because the older aggregation reader requested an absent `mean_nll` field from the B84S-Q fold metrics (`KeyError`). This is a schema-only repair, not a scientific result: the failed invocation produced no aggregate/event JSON or completion marker and did not overwrite existing artifacts. The aggregation now includes only metrics present in every completed fold; the frozen replay is rerun under the new `_b84sq_v3` suffix.
- B84S-Q frozen event replay then completed over all 760 records (76 positive + 76 negative at prefixes 1/2/4/8/16). The repaired-contract formal validation weighted candidate top-1/top-5 recall was `0.067961/0.626319`, candidate-or-DEFER accuracy `0.590969`, and DEFER recall `0.689947`; no controller or held selection was involved. At prefix16, learned support selected a candidate on 26/76 positives and 19/76 negatives, but only 7/76 positive and 2/76 negative selections reached posthoc IoU>=0.5; same-space raw source-mean reached 20/76 positive and 8/76 negative reliable selections, while frozen Q0 both-side reliability was 25/76 positive. Every event had a nonempty native candidate set (median 196.5, mean 272.4 candidates), so this is not an empty-pool failure. Among positive events, 27 had frozen source observability failure, 24 target observability failure, 18 deferred despite a reliable target, and only 4 selected reliable events also passed the frozen source/target reliability flags. The dominant actionable issue is query-conditioned selector generalization/calibration over the repaired three-fold TRAIN contract; no further B84S model variant or controller run is authorized in this window.
- Because the raw same-space diagnostic remains materially stronger than the learned B84S-Q selector, one fixed B84S-RA raw-anchor diagnostic is registered (configuration and preregistration in `configs/iclr27_phase84/b84s_raw_anchor_v1.json` and `docs/iclr27_phase84/PHASE84_B84S_RAW_ANCHOR_PREREGISTRATION.md`). It adds a non-tuned `0.05*tanh` within-set residual to raw source-mean cosine and falls back to the raw candidate when the frozen model emits DEFER; it does not train, alter candidate sets, or run a controller. The first compile exposed a missing closing bracket in the new evaluator branch; no run or artifact was created, the bracket was repaired minimally, and compilation is rerun before this single diagnostic.
- B84S-RA completed over the frozen 760-record event manifest. At prefix16 it selected a candidate on 76/76 positive and 76/76 negative events, with posthoc reliable selections 24/76 positive and 9/76 negative versus raw source-mean 20/76 and 8/76. Per event fold positive reliable counts were [1,4,9,10] versus raw [1,3,8,8]. The bounded residual therefore recovers a modest +4 positive events but also increases negative activation by one; it remains below the registered >30/76 meaningful selection criterion and is not an O-support or controller result. The read-only support-alignment callgraph confirms no transformed support IoU exists in B84S/B84S-Q/B84S-RA, so alignment is not authorized; the remaining evidence is selection/ranking generalization rather than a transformation stage.
- The B84S-Q failure audit and B84S-RA comparison now include event-level taxonomy and category/source/target-video coverage in `outputs/iclr27_phase84/audit/{b84sq_failure_audit,b84sra_failure_audit}.json`. A lock-aware live-ledger generator and final-report generator were added and compiled/pushed; the final generator correctly refused to write the report while the registered Phase84 finalization lock remained closed. No controller, alignment, or sealed evaluation was started after the B84S-RA diagnostic.
- A single registered B84S-PROTO diagnostic then selected each native candidate by the maximum cosine to the fixed three contiguous causal source prototypes (no training or threshold). At prefix16 it obtained 23/76 positive and 11/76 negative reliable selections, below B84S-RA's 24/76 and with higher negative activation. Its event taxonomy is retained at `outputs/iclr27_phase84/audit/b84sproto_failure_audit.json`; the >30/76 alignment-routing criterion remains unmet, so no alignment or controller route is opened.
- Phase84 finalization completed after the registered deadline-minus-45-minute lock opened at `2026-09-05T03:05:55Z`. The report, decision, provenance, live ledgers, and hashes parsed successfully; no Phase84 process remained, no public/DEV+/Q1/sealed labels were accessed, and the finalizer commit was pushed to `origin/main` (`92ebb27`). Status remains `AUTONOMOUS_PHASE84_COMPLETE_WITH_INTERFACE_NEGATIVE_EVIDENCE`; controller, Commit-CT, and sealed evaluation were not authorized because the physical-R and source-selection gates failed.

## Phase85 autonomous repair window (2026-09-05)

- Registered a fresh ten-hour Phase85 window from `2026-09-05T08:50:01Z` to `2026-09-05T18:50:01Z` (finalization unlock `18:05:01Z`) from synchronized HEAD `e4e2f070d17adbdce290a2b418990143452e9522`. Outputs use the independent `/data2/usr_for_deadline/trackocd_phase85/project_outputs` target through `outputs/iclr27_phase85`; no public DEV+/Q1/new-model/sealed labels were accessed. GPU0 was occupied by an unrelated process, while GPUs1--7 were idle at the initial snapshot; Phase85 scientific work used bounded CPU workers and did not terminate external processes.
- Phase84 issue audit and repair ledger are retained at `outputs/iclr27_phase85/audit/phase84_issue_audit.json` and `repair_events.json`. The repaired implementation uses running `app_sum/app_count/app_mean`, correct causal union inheritance, real Q0 adapter reconstruction (not `raw - raw`), single causal anchor roots, explicit native/public join modes, provenance assertions, and an early-finalization guard. The corrected temporal-mean full replay preserved `682,335` rows/`370` videos (lineage SHA `7bf5f00363b69d6e992f42e5ea165c5b870927338f2dd242847c081072e99bf0`; union SHA `9dd7eb048268859e29202138f15fbf4bf3b2ea849bd39434b4c8c1a5ae02fe4b`).
- Real Q0 adapter parity (`q0_parity_v5`) passed on the frozen `984`-query universe with max vector error `5.96e-08` and zero bad queries. Its join covered `43,398/43,423` public rows (43,319 exact, 79 deterministic IoU fallback, 25 unmatched); the failed v1--v4 parity artifacts are preserved. The single-anchor temporal-mean adapter is explicitly diagnostic (`max error .3578`, 3,935 bad) and does not replace the Q0 anchor.
- Same-protocol class-agnostic TrackEval on 91 event videos showed the repaired temporal mean modestly above Q0 on HOTA/IDF1 (`14.743/7.991` vs `14.619/7.749`), with AssA `32.811` vs `32.246`, IDSW `2614` vs `2647`, Frag `748` vs `748`, DetA `6.807` vs `6.815`, MOTA `-821.98` vs `-822.32`. This is a physical diagnostic only; the full Q0 physical stream and MOT protocol remain frozen.
- Frozen physical-to-R comparison (984 queries, same candidate order/exclusion) remained negative: temporal mean p16 R@1/mAP/hard-gap `0.8186/0.8118/0.1304` vs Q0 `0.9000/0.8599/0.1944`, 22 unsafe flips and zero folds non-decreasing in both metrics. The selective TRAIN-only union gate was then run as the one registered safety branch; it preserved all rows (lineage SHA `33f8090ebeb26bd3e8273dddde54addfda8ef3141f7a5aad5d230daae7d5f508`, union SHA `79d68427998a95d257b2bc6c40f6dbee80ccc014e511b07f794c67b7c96c21c8`) and improved event91 HOTA/IDF1 slightly (`14.658/7.849`), but selective p16 R@1/mAP/hard-gap `0.8149/0.8065/0.1289` remained below Q0 with 22 unsafe flips. No physical-to-R route authorized a controller.
- Phase85 support manifest uses fixed raw-top32, all prefixes `{1,2,4,8,16}`, legal TRAIN-only source/query groups and a deterministic three-fold disjoint split (`419` groups per prefix; fit/validation counts are retained in the manifest). TRAIN top-K audit selected fixed `K=32` because validation top-16 oracle coverage was below the pre-registered 90% rule. Support contract smoke and `smoke_r2`/`targeted_r2` passed finite-output checks; the old polarity bug and the first system-Python invocation are preserved as implementation evidence, not scientific results.
- Formal raw-anchored reranker/DEFER training completed three bounded CPU folds (15 effective epochs, atomic checkpoints, no OOM): fold validation bridge-use/defer metrics were f0 `.125/.848`, f1 `.294/.632`, f2 `.156/.760`; reranking did not dominate the raw anchor. Frozen 76+76 replay at p16 gave raw/reranked/final reliable events `20/23/8` positive and `8/15/8` negative (source/target frozen coverage `49/40`, both `25`; final deferred `58` positive, `47` negative). The final DEFER policy therefore fails safety; controller and alignment were not run.
- The physical selective union gate was trained TRAIN-only on `1,359` examples across three bounded folds (formal accept precision/recall/false reconnect approximately `.422/.687/.549`, `.411/.599/.534`, `.418/.601/.497`). An initial per-row Torch replay was terminated only for task-owned PIDs `32861/32862` after profiling showed an avoidable performance bottleneck; the NumPy MLP path was smoke/targeted tested and the formal full replay completed without OOM or external termination. The event-level support selection audit (`support_selection_audit.json`) finds at p16 381/690 positive rows and 350/690 negative rows with a pool IoU>=.5 candidate, but many are not selected; this separates candidate availability from rank/defer errors.
- A registered P1+S0 combination using a selective-lineage source cache was run once. The cache covers `1,298` tracks (p16 no fallback), but support replay fell to `8/76` positive and `6/76` negative raw reliable rows (reranked `12/76` and `8/76`; final `6/76` and `5/76`). This closes the physical-source transfer hypothesis for this window. The TRAIN/event distribution audit is in `support_train_event_shift.json`; it is diagnostic and did not alter thresholds, denominators, seeds or checkpoints.
- Phase85 scientific state is therefore: repaired physical implementation and true parity are valid; temporal/selective physical variants do not safely improve frozen R; raw-anchored learned support adds a small positive selection but increases negative activation and DEFER harm; selective-source support is worse. No alignment, controller, StateMemory, threshold sweep, backbone download, or sealed/public evaluation is authorized. Remaining work is provenance-safe ledger/report generation inside the registered finalization interval, followed by a GitHub push of the final report.
- Additional event-video root audit (`event_physical_contamination.json`) mapped 9,709 public event rows. Multi-category canonical-root fractions were Q0 `0.0501`, temporal mean `0.0776`, selective `0.0695`; temporal changed 2,520 rows versus Q0 and selective changed 1,545. This confirms that the physical variants' modest TrackEval gains coexist with increased semantic contamination, supporting the frozen-R safety failure rather than a controller diagnosis.
- The first contamination-audit pass exposed a zero-valued frame/proposal key coercion and a temporal-root field-name mismatch; no artifact was trusted from that pass. Preserving zero with an explicit `as_int` helper and mapping `temporal_root` consistently yielded the corrected 9,709-row result above; this was a diagnostic-only repair with no protocol or scientific input change.
- Phase85 report/provenance preflight now passes all 12 route/tag assertions (including the leakage and raw+DEFER diagnostics); the integrity audit parses 79 JSON artifacts, 133 checkpoints, and reports no forbidden output names or residual Phase85 process. The single failed physical-gate smoke marker remains intentionally uncompleted and is not relabeled. Early finalization is still blocked until the registered `2026-09-05T18:05:01Z` interval.
- Phase85 leakage-contract audit completed from the three TRAIN-derived manifests (`leakage_contract.json`): status PASS; public DEV+/Q1/new-model/sealed access, future rows/tracks, and ID-as-model-input flags are all false, and the source scan found no non-declaration forbidden-input mentions. The audit is provenance-linked as a final-report section; it does not change any scientific route or authorize controller/sealed evaluation.
- Following the registered fallback for a reranker that harms the raw anchor, a raw-ranking-plus-trained-DEFER replay (`support_event_replay_raw_defer_v1.json`) was run once with the reranker score ignored. At p16 it retained 5/76 positive and 2/76 negative reliable events after 58/47 deferrals, versus raw 20/8; this also fails anchor preservation and does not authorize alignment. No training, threshold change, controller or sealed evaluation was performed.
- The lock-aware finalizer was strengthened before unlock so that, after its single finalization commit/push, it records `final_repository_head` and `finalization_commit` in both `finalization_lock.json` and `phase85_decision.json`; the report's separate `SCIENCE_HEAD`/`REPORT_GENERATION_HEAD` remain unchanged and auditable.
- The live ledger now captures a low-frequency `nvidia-smi` snapshot alongside `free -h`, disk and process counts; no worker was launched by this audit and the remote `main` remains synchronized.
- Phase85 finalization completed inside the registered `2026-09-05T18:05:01Z` unlock interval. The atomic report and decision were generated from the 12-section provenance ledger, pushed in commit `e47effc0b8265e13ec41457aba5c7e79a02e0ff5`, and the post-commit decision/lock record the same `final_repository_head`. Final integrity is PASS (79 JSON artifacts, 133 checkpoints, zero parse/missing/process errors); the preserved failed smoke marker remains evidence. No controller, alignment, threshold, backbone, public/DEV+/Q1, or sealed evaluation was run because all authorized Phase85 gates were negative/unsafe.

## Phase86 frozen diagnostic and selective-support window (2026-09-06)

- Registered the Phase86 window from `2026-09-06T07:57:11.490953Z` with a ten-hour deadline in `outputs/iclr27_phase86/audit/window_registration.json`. The repository started at `43a8c4e248f58b81deb0b831084282603e7bf1d6`; GPU0's unrelated PID 33785 was left untouched, GPUs1--9 were idle in the registration snapshot, and large outputs were placed under `/data2/usr_for_deadline/trackocd_phase86/project_outputs` through the project symlink.
- Historical controller discovery selected the chronologically last byte-identifiable formally frozen RC-MS-OCD Phase19R controller and StateMemory, with the four fold checkpoint hashes in `manifests/frozen_controller_manifest.json`. This choice was made from chronology/formal-use provenance, never from Phase86 event performance. Phase56's unified controller remains negative historical evidence.
- The required upstream order was frozen before reading new diagnostic outcomes in `audit/upstream_plan_preregistered.json`. Diagnostic outputs are isolated from all training/checkpoint-selection inputs (`audit/diagnostic_isolation.json`).
- The first diagnostic invocation failed on the Phase86 import path; the second completed D0/D1 in memory but failed to commit the summary because the read-only Phase85 artifact path was resolved under the wrong directory. Both failures are retained as `completion/diagnostic_ocd_r0_failed.json` and `diagnostic_ocd_r1_failed.json`; the smallest path-only repair was applied and a successful replay produced `diagnostic_ocd/summary.json` and `event_traces.jsonl`.
- Frozen diagnostic OCD D0 historical Q0 and D1 Phase85 temporal physical both produced `3/76` Commit-CT, all three in fold3; D2/D3 were explicitly marked interface-incompatible because Phase85 score-level artifacts do not expose the frozen controller's causal 768-D row-vector contract. This is diagnostic-only, not model selection or a formal OCD result. The D0-to-Phase72 parity artifact records fold-level lineage but correctly marks protocol equality false because the legacy JSON has a different event-accounting representation.
- U1 used only OOF TRAIN predictions from the frozen Phase85 raw/reranker experts. The 64-D feature vector, utility labels (`HELP=+1`, `HARM=-4`, otherwise `0`), checkpoints, and 2,095-row meta manifest are recorded under `outputs/iclr27_phase86/manifests/` and `metrics/`. The first smoke exposed a scalar/list counter bug before metrics; its `.launched` and `.failed` evidence are retained. Fresh smoke and fold0 targeted runs passed after the smallest counter-type repair. Three formal OOF folds completed without GPU/RAM pressure: validation net rescue/harm were f0 `+1/1`, f1 `+2/1`, f2 `-1/3`, so the TRAIN gate passed on 2/3 folds.
- U1's one fixed 76+76 event replay did not meet the registered event safety status: p16 raw was `20/8` positive/negative reliable events, full reranker `23/15`, and selective U1 `22/12`, with the gate using the reranker on 66/76 positive and 58/76 negative events. Therefore no controller compatibility or sealed run was authorized; this is a score-level intervention result, not Commit-CT success.
- The registered parameter-free Root-of-Fragments diagnostic used frozen Phase75D vectors, fixed causal contiguous four-frame fragments, symmetric Chamfer matching, and a fixed `0.05*tanh` raw anchor. It produced p16 `R@1=0.893219` vs raw `0.893219`, `mAP=0.848641` vs raw `0.848374`, zero unsafe flips, and only 2/4 folds non-decreasing in both metrics. Decision is `ROOT_OF_FRAGMENTS_V1_NEGATIVE`; no controller, threshold, StateMemory, public, DEV+, Q1, or sealed labels were accessed.
- Phase86 current decision is retained for final reporting: the frozen controller diagnostic is complete; U1 has TRAIN evidence but fails the registered event-level safety status; RF is negative; U2 remains unopened because the preregistered trigger is a failed U1 TRAIN gate, not an event-only score failure. No scientific artifact was selected from diagnostic OCD, and no public/sealed evaluation was run.

## Phase86 resume: deployment/RF contract repair (2026-09-06)

- The Phase86 report had been generated prematurely at about 40 minutes even though the registered ten-hour window remained open. The original registration was not rerun. `outputs/iclr27_phase86/audit/resume_after_premature_finalization.json` records the original start/deadline and this correction; `execution_policy.py` now blocks report generation until deadline-minus-45-minutes or an explicit allowed hard blocker. The old report remains an interim snapshot.
- The previous event-fold-modulo-3 U1 replay and within-track four-frame RF result were reclassified as diagnostics. A continuation amendment freezes the candidate universe, seeds, denominators, and sealed boundary and authorizes the corrected deployment plus U2.
- Corrected U1 deployment trained one `SupportReranker(19,10,64,0.05)` on all 2,095 non-event TRAIN groups for 15 effective epochs (54,780 steps, seed 86001), then one unchanged `UtilityGate(64→32→LayerNorm→Tanh→1)` on all 2,095 OOF TRAIN rows for 2,000 steps (seed 86100). The first U1 trainer attempt failed only while hashing an already-written checkpoint; the failure marker is retained, and a fresh `_fix1` tag completed. The formal all-event replay uses only the two final all-TRAIN/all-OOF checkpoints and no event-fold model routing.
- Corrected U1 p16 replay produced raw/full-reranker/final reliable events `20/20/20` positive and `8/15/12` negative; 66/76 positive and 71/76 negative events used reranking. Registered U1 event safety (`positive >=22`, `negative <=8`) therefore fails. No controller or sealed result was claimed.
- Canonical-root RF was rebuilt from the Phase85 `full_temporal_lineage.jsonl` and `union_events.jsonl`, replaying unions only up to the causal anchor. It reconstructed 3,527,920 comparisons, 24.6% roots with >1 fragment, maximum six fragments, and 830 fallbacks. Raw p16 parity is `R@1=.893219`, `mAP=.848374`; canonical-root p16 is `R@1=.893219`, `mAP=.849139`, hard-gap `.184725` vs `.189559`, but one unsafe flip and only 2/4 folds non-decreasing. Decision: `RF_CANONICAL_ROOT_V1_NEGATIVE`; this closes RF-v1 only.
- Phase19R controller discovery now records the real Phase56 source hash (or missing status) and historical byte-identical commit `686d6194debe127ca7758f698f96dc59b020b042` for both controller and StateMemory. D0/D1 were not rerun; their complete metrics are aggregated from frozen summary/traces into `audit/diagnostic_ocd_full_metrics.json`.
- U2 set-aware relation v1 was trained for 15 epochs on each fixed three-fold TRAIN split. Validation rescue/harm were `(0,0)` in all folds and raw top-1/top-5 were unchanged; checkpoint residuals saturated to a constant `-0.05`. A single evidence-based repair balanced match/defer group exposure without changing architecture or protocol; it also yielded `(0,0)` rescue/harm in all three folds. Decision: `U2_BALANCED_TRAIN_NEGATIVE`; no third U2 variant, event replay, controller, or sealed evaluation is authorized.
- A verified read-only recent-method audit covered AGE, TALON, LTC, TRACT, COVTrack, ObjectRelator, and C3Po with current heads/licenses where available. None exposes a drop-in text-free causal prior-support selector for this Phase86 schema; no external code/weights were imported. Public DEV+/Q1/new-model/sealed labels, future rows, IDs, and text remain sealed from inference/training selection. GPU0 PID33785 and external intermot workers were not touched; CPU routes completed without OOM.

## Phase87 causal persistent OCD controller redesign (2026-09-06 onward)

- Registered a fresh ten-hour Phase87 window from `d6c193e` at `2026-09-06T17:53:10Z`, deadline `2026-09-07T03:53:10Z`; output is `/data2/usr_for_deadline/trackocd_phase87/project_outputs` through the `outputs/iclr27_phase87` symlink. GPU0--4 were occupied by external processes, so planned task cards are GPU5--8; no external PID was touched.
- Phase86 evidence repairs are isolated in the Phase87 namespace. U2 frozen checkpoints were re-evaluated with `model.eval()` and `torch.no_grad()` without retraining; all six validation folds remained exactly raw (`rescue=0`, `harm=0`), confirming `U2_COMPRESSED_FEATURE_COLLAPSE_CONFIRMED`.
- The corrected RF replay now maps `(frame_id,image_id)` to ordinal observed steps per video for both union events and fragment rows. It produced p16 `R@1=.893219`, `mAP=.849139` versus raw `.893219/.848374`, one unsafe flip and only 2/4 non-decreasing folds; decision `RF_PHASE86_OBSERVED_STEP_CORRECTION_NEGATIVE`. The original Phase86 files were not modified.
- A direct-script import-path failure occurred before the first U2/RF rerun; it was repaired minimally by adding the repository root to `sys.path`, then the same paths completed. No training, public/DEV+/Q1/sealed labels, future rows, IDs or text inputs were used in these corrections.
2026-09-07 Phase87: controller contract check passed after correcting the action context to 271 dimensions (256 track + 7 scalar + 8 support); joint action space includes NEW/DEFER/RESET, causal candidate AND rule, monotonic source-before-target rollout, and non-zero gradients. Added a joint-interface mask so empty prototype slots cannot become random EXISTING actions. The desktop executor briefly left three duplicate contract-check processes alive while the blocking wrapper returned early; all were task-owned, explicitly terminated except for the oldest run, and the final artifact was preserved. That oldest run also hung in a large Torch thread-pool after writing a valid PASS artifact, so it was explicitly terminated; no training or external process was touched. Contract checks now cap Torch threads to two.
2026-09-06 Phase87 C0 validation: all four 20k folds completed. TRAIN validation commit recall was 0.1564/0.1978/0.2000/0.2973 for folds 0–3; negative false-merge was 0.3216/0.5300/0.0562/0.0444, so this is diagnostic evidence, not a passed formal gate. The first held-event replay attempt stopped at fold0 because the frozen Phase19R manifest uses `target_tracklet_key` while the new evaluator expected `target_track_key`. This is an interface-only root cause; no held result was written or selected. A normalizer was added and the failed log is preserved; replay will use a new repair tag.
2026-09-06 Phase87 C0 formal: four 20k folds completed on GPUs5–8. TRAIN validation improved over an all-DEFER comparator in all folds, but frozen diagnostic replay was 19/76 Commit-CT, existing precision .1450, negative false-merge .1711; C0 formal gate failed. One preregistered repair raised false-merge weight 2→3. Repair1 gave 17/76, precision .1250 and false-merge .1447: the safety rate barely passed but recall/precision worsened, so the C0 route was stopped.
2026-09-06 Phase87 C1 support integration: because C0 TRAIN validation had positive learning signal, a single support route was allowed. Eight support inputs were causal source/target similarity, fixed candidate count, source length/variance, observation quality and history consistency; no IDs/categories/text/future. C1 smoke/targeted passed; four 20k folds completed and diagnostic replay gave 18/76, existing precision .1731, negative false-merge .0789, with known micro/macro not applicable to this novel-only replay. C1 formal gate failed on precision and therefore unchanged-controller compatibility/sealed evaluation were not run. No public/DEV+/Q1/sealed labels were accessed.

## Phase88 persistent OCD contract repair (2026-09-07)

- Phase88 H1 TRAIN-only false-merge/reset repair was registered after the C0/C1 fair-control audit: C0 TRAIN negative false-merge `.699529` and premature `.643164` dominated; C1 reduced false-merge to `.500512` but raised unresolved to `.216148` and lost the frozen TRAIN selection score. H1 changes only the registered loss profile (`false_merge_risk=5.0`, `new_existing_margin=1.25`, `reset_margin=1.25`; other weights unchanged); StateMemory, transitions, thresholds, manifests, seed, denominator and evaluator remain frozen. Registration: `outputs/iclr27_phase88/audit/hypothesis_1_preregistration.json`.
- H1 smoke (100) and fold0 targeted (500) passed with finite decreasing loss and nonzero RESET supervision. The first formal supervisor launch failed before training because shell command substitution made background PIDs non-children; the second launch exposed an incorrect memmap path. Both failures are preserved in `logs/h1_false_merge_reset_supervisor.log`; the script was minimally repaired and smoke/targeted rerun.
- H1 formal then ran three bounded workers from the frozen C0v2 checkpoints. At available RAM ≈28 GiB (<31.25 GiB floor), only task-owned PIDs `35717,35718,35721,35722,35723` were explicitly stopped; external jobs were untouched. Checkpoints through f0 step18k, f1 step22k and f2 step20k are retained with hashes in `outputs/iclr27_phase88/audit/h1_resource_stop.json`; no H1 `.done` or formal validation was claimed. Next action is sequential one-worker continuation from these valid checkpoints, with no relaunch of the original launched units.
- User-requested project-local storage cleanup archived the unused historical Phase54/69/70 outputs and regenerable `data/caches/features` to `/data2/usr_for_deadline/trackocd_archive/20260909/project_cleanup/`, preserving original paths as symlinks. Manifest, reports, checkpoints and active Phase88 outputs were retained; no external process was touched. `/data1` availability increased from about 34 GiB to 42 GiB. Ledger: `outputs/iclr27_phase88/audit/storage_cleanup_20260909.json`.
- During the sequential H1 continuation, f0 and f1 reached 30k `.done`; f2 reached step28k before available RAM again fell to ≈29 GiB. Only task-owned PIDs `4157,4159,13951` were explicitly stopped; f2 step28k checkpoint and `.launched` marker remain, no formal validation was selected. Resource evidence: `outputs/iclr27_phase88/audit/h1_resume1_resource_stop.json`; external jobs were untouched.
- After the next safe window, H1 f2 completed 30k and f3 reached step28k, but available RAM again fell to ≈28 GiB. Only task-owned PIDs `39627,39629,18522` were explicitly stopped; f3 step28k checkpoint and `.launched` marker remain, with no formal validation yet. Resource evidence: `outputs/iclr27_phase88/audit/h1_resume2_resource_stop.json`; external jobs were untouched. The final 2k f3 continuation remains pending a safe RAM preflight.

- Registered Phase88 from clean main `3c12af72a83681883cd6e4e5ec0beeb35d78bdd3` at `2026-09-07T06:20:15Z`; old Phase87 files are read-only and outputs are isolated at `/data2/usr_for_deadline/trackocd_phase88/project_outputs` via the project symlink. Initial resource snapshot found external workers on GPU1--4; Phase88 selected idle GPU5--8, with no external process touched.
- Phase88 EventV2 builder first hit a direct-script import-path failure (`ModuleNotFoundError: src`); the smallest repair added the repository root to `sys.path`, and the rebuilt manifest completed. It contains visual-cosine hard negatives, 2--4 source tracks, no numeric video chronology, and fold0 TRAIN scarcity (270 legal events, below the registered 500 target) recorded rather than duplicated. A fixed 10% TRAIN-only wrong-binding augmentation yields RESET targets; validation remains unaugmented.
- Formal contract checks passed: Phase19R `metrics` is the direct metric source; known_count=48 (fold0 active=7) with prototype hash `3282127d7e901332a9b0080910365d7a8c924ca622d04fa92c0a7b586cf78704`; all source tracks are processed; StateMemoryV2 is used; RESET target count in smoke is 20; no future rows, IDs or text enter model tensors.
- Phase87 full-runtime replay was first attempted on CPU but became an 8-minute task-owned process without producing a result, so explicit PID `36793` was terminated for performance safety and replay was rerun on idle GPU5. Corrected full-runtime replay produced real Phase19R-schema metrics: old C0/C1 checkpoints did not reproduce the Phase87 headline (C0 fold3 only 1/28 CT; all other folds 0; C1 aggregate 0), and known metrics are now numeric (not NA), exposing the prior rollout/evaluator mismatch.
- C0v2 smoke initially failed with an undefined `support_mode` argument; the failed `.launched`/log are retained. The minimal signature repair passed 100-step smoke and 500-step fold0 targeted: bf16 finite, nonzero gradients, loss decreased, RESET supervision was sampled. A four-fold 20k C0v2 supervisor was launched on GPU5--8, but at about 6k updates system available memory fell to ~21 GiB (<25% safety floor). Only task-owned PIDs `25608`--`25611` and supervisor `25602` were SIGTERM/KILLed; external workers were untouched. All completed step checkpoints and `.launched` markers were retained. Next action is bounded two-worker resume from latest valid checkpoints with RNG/checkpoint persistence repair; no scientific gate or public/sealed result has been claimed.
- Phase88 repair2 pair resume was launched with task-owned supervisor `39830` and workers `39839` (fold0/GPU5) and `39844` (fold1/GPU6), resuming the latest valid fold checkpoints. System available memory fell to about 27 GiB while the pair was active, below the 25% safety floor; no new repair2 checkpoint was written. Only these three task-owned PIDs were explicitly terminated, preserving both `.launched` markers and empty logs. GPUs5/6 were idle after cleanup, external workers were untouched. The pair route is retained as a resource-safety failure; next continuation must use one bounded worker and must not relaunch the already-launched repair2 units.
- An intermediate full validation attempt on `c0v2_repair1_f0_step006000.pt` was started on idle GPU5 with task-owned shell/Python PIDs `14241/14243`. After more than ten minutes at high CPU and no atomic metrics artifact, it was explicitly terminated for evaluator performance; no checkpoint or metric was selected or overwritten. The cause is evaluation cost (each event replays multiple causal tracks and the current exact evaluator recomputes the known stream), not a model result. The failed log and empty validation directory are retained. A bounded representative validation path is required before any further training decision.
- The bounded 100-event TRAIN-disjoint replay of fold0 step6000 completed atomically on GPU5. It produced Commit-CT `2/51` positive events (sample denominator 51), existing precision `.1818`, category coverage `2`, video coverage `2`, negative false-merge `.1429`, premature `.73`, duplicate births `24`, known micro/macro `1.0`. This is an intermediate diagnostic, not a formal gate; the high premature rate and low reuse signal do not justify claiming success. The full evaluator remains reserved for completed checkpoints.
- A single-worker `c0v2_repair3` continuation was registered to avoid the repair2 memory event. It uses GPU5 only, explicit 25% RAM-floor preflight, `MALLOC_ARENA_MAX=1`, and resumes f0/f1/f2/f3 from the latest valid partial checkpoints under new tags; already-launched repair2 units are not relaunched. Formal 20k completion remains required before any C0v2 gate decision.
- Repair3 f0 reached and atomically saved `c0v2_repair3_f0_step008000.pt` after about 31 minutes, but available RAM fell to ~31 GiB (<25% of 125 GiB). Only task-owned PIDs `30223` (worker), `30216` (supervisor), and `30214` (wrapper) were explicitly SIGTERM/KILLed; after cleanup available RAM recovered to ~36 GiB and GPU5 was idle. The repair3 `.launched` marker and checkpoint remain; no `.done` or formal metric exists. This is the third bounded C0v2 repair/resource attempt, so the C0v2 training route is exhausted for this window; no C0v2 gate, C1 support route, controller compatibility, or sealed evaluation is claimed.
- Phase88 final decision is `P88_C0V2_FORMAL_INCOMPLETE_RAM_FLOOR_STOP_NO_CONTROLLER`. The report and machine-readable decision preserve contract PASS, corrected Phase87 replay evidence, smoke/targeted metrics, the bounded partial validation, all resource PIDs, checkpoint hashes, and the explicit distinction between an incomplete route and a scientific failure. No public/DEV+/Q1/sealed labels were accessed and no external process was touched.
- Phase88 immediate-repair/resume was authorized without registering a new phase. At `2026-09-07T10:18:00Z`, HEAD was `378456f1c58156ab4e15e2b77749b688574ea816`; the original window/deadline remains unchanged. The previous report is reclassified as an interim resource-stop snapshot. All old C0v2 checkpoints are marked `INVALID_FOR_FORMAL_CONTINUATION` because their target-semantics contract was wrong; they remain read-only provenance and will not initialize fix2 training. Current external GPU0/1-4/6-9 workers were left untouched; GPU5 is the only available Phase88 card.
- Fix2 implementation repair completed: source/target role semantics, atomic teacher forcing, category-complete pseudo-novel masks, wrong-binding RESET filtering/local-slot mapping, TRAIN/runtime collected-vector parity, RESET/DEFER loss routing, transition-field clearing, uncached FeatureStore, shared read-only memmap, fold0 variant expansion, dynamic execution/resource policy, measured RSS, and sharded atomic validation. A direct build import failure (`ModuleNotFoundError: src`) was minimally repaired; the corrected shared store and fix2 manifest then completed. Fold0 expanded to 982 unique fit events (variants=16), above the 500 target.
- Fix2 contract check passed: source polarity mismatch=0, teacher atomic=true, masked target violations=0, pseudo-novel mask complete, RESET target present, collected-vector trace present. Scratch smoke r2 (100) and targeted (500) were finite with non-zero gradients and stable RSS (~5.05 GiB after model/step500). The bounded 100-event TRAIN validation is diagnostic only: Commit-CT 8/51, existing precision .3158, category/video coverage 2/8, negative false merge .8367, premature .77; it was not used to select a model.
- Fix2 formal f0 was launched from scratch on GPU5 using the memmap store. At step2000 it wrote `c0v2_fix2_formal_f0_step002000.pt`; available RAM then fell to ~31 GiB, below the 31.25 GiB safety floor, while the worker RSS was stable near the measured 5.05 GiB. Only task-owned PIDs `1368` (worker), `1367` (supervisor), and `1365` (wrapper) were explicitly stopped. No `.done` or validation artifact was written; the `.launched` marker and checkpoint remain. With the corrected memory-bounded store and one worker still crossing the floor under the external host load, the current-window blocker is reclassified `UNRECOVERABLE_RESOURCE_BLOCKER` (execution-resource scope only). No C0v2 formal gate, C1 support route, controller compatibility, or sealed evaluation is claimed.

- Phase88 immediate-repair closure record: `outputs/iclr27_phase88/audit/fix2_resource_stop.json` and the updated `final_decision.json` now classify the current window as `PHASE88_FIX2_UNRECOVERABLE_RESOURCE_BLOCKER_CURRENT_WINDOW`. Fix2 formal was genuinely restarted from scratch after the target-role/teacher-forcing repair; old checkpoints were not used. The corrected memmap single-worker route still crossed the mandatory RAM floor at fold0 step2000. The launched marker and atomic checkpoint SHA256 `9bc85fa45022262270752abb25b0068efc9f7c0178074477d2cc9cd06fa68f3d` are retained; no `.done`, formal validation, C1, controller, or sealed artifact exists. The fix2 smoke/targeted checkpoint hashes are `1ac8d573cdd6984375ead9654e3d1c970cf4b3193d20178a33ae879c1e2df726` and `98b608036fd1206419a460d517b798078876a631fef0b8b2ded7c7e527667304`. No external process was touched and public/DEV+/Q1/sealed labels remain sealed. A newly authorized memory-safe window or isolated host is required before any formal continuation; do not relaunch the existing `.launched` unit in this window.

- Phase88 continuous-completion transition event: the old supervisor PID `36508` was explicitly stopped to replace its deadline-bound code with the completion-based supervisor, but the PTY-bound task worker PID `36612` became a zombie before its next atomic checkpoint. This is recorded in `outputs/iclr27_phase88/audit/supervisor_transition_resource_event.json`; the only recoverable progress remains fix2 step2000 (SHA256 `9bc85fa45022262270752abb25b0068efc9f7c0178074477d2cc9cd06fa68f3d`). No external process was touched, no scientific checkpoint was selected, and the new supervisor must resume from that checkpoint while preserving the existing `.launched` marker.

- The required low-RAM TRAIN-only analysis was generated at `outputs/iclr27_phase88/audit/low_ram_failure_analysis.json`. It uses only the fix2 targeted 100-event TRAIN-disjoint records and resource evidence: loss `3.3833→1.6334`, reset targets `504`, masked-target violations `0`, source action counts, prefix commit distribution, existing/new logit margins, negative false-merge `.8367`, premature `.77`, unresolved `.06`, and duplicate births `9`. It is diagnostic and cannot select a model or access held 76+76 labels. The completion supervisor remains `IN_PROGRESS` and is waiting/working toward formal four-fold completion.

- Phase88 H2 known-branch suppression route was registered after H1 was rejected by TRAIN-only selection. H2 adds only the preregistered non-KNOWN versus strongest-KNOWN logit safety term (`known_suppression=1.5`); manifests, StateMemory, action semantics, seed, denominator, and event tag remain frozen. H2 100-step smoke and 500-step targeted runs passed with finite loss and nonzero known-suppression loss.
- The first H2 formal three-worker window reached f0/f1/f2 steps 18k/20k/18k before `MemFree=32,642,232 kB` fell just below the mandatory 25% RAM floor. Only task-owned PIDs `30846,30847,30848,30843,30827` were explicitly SIGTERM'd; checkpoints and `.launched` markers were preserved, external processes were untouched, and no validation/held result was selected. Evidence is `outputs/iclr27_phase88/audit/h2_resource_stop.json`.
- H2 continuation is now bounded to one sequential worker with new resume tags, after Python/bash smoke checks. It resumes the latest valid H2 checkpoints (f0 18k, f1 20k, f2 18k), then trains unfinished f3 from the frozen C0v2 checkpoint; no existing launched unit is relaunched. H2 remains TRAIN-only and no public/DEV+/Q1/sealed labels have been accessed.
- The one-worker H2 continuation reached f0 step22k, then MemFree fell to about 29.7 GiB below the 25% floor. Only task-owned PIDs `22305,22303,22297` were explicitly SIGTERM'd; f0 step22k is retained with hash in `outputs/iclr27_phase88/audit/h2_resume1_resource_stop.json`, no `.done` or validation result was claimed, and external jobs were untouched. A second RAM event means the remaining H2 formal completion is resource-blocked until a verified safe host/window; incomplete H2 must not be selected.
- A minimal resource repair made Torch/BLAS thread counts configurable and lowered them to one for H2 continuation. Resume smoke 22,000→22,010 and fold0 targeted 22,000→22,500 both passed with finite loss, nonzero gradients, RESET targets and zero masked violations. A formal resume2 attempt still reduced MemFree to about 25.5 GiB before its first checkpoint; only task-owned PIDs `37167,37165,37161` were stopped and no new formal checkpoint/metric was selected. Evidence is `outputs/iclr27_phase88/audit/h2_resume2_resource_stop.json`; H2 formal remains resource-blocked under the current host load rather than scientifically evaluated.
- The first CPU low-memory smoke exposed a legacy CPU `load_state_dict` segfault when the model buffer aliased a read-only memmap mask. The failed tags `h2_known_suppression_cpu_smoke_f0` and `_fix1_f0` are preserved; a minimal writable-buffer clone plus explicit CPU tensor-copy repair passed CPU smoke and targeted (`h2_known_suppression_cpu_smoke_fix2_f0`, `h2_known_suppression_cpu_targeted_fix2_f0`) with RSS about 0.45 GiB, finite FP32 loss, nonzero gradients and zero masked violations.
- A four-worker CPU formal attempt then drove MemFree to about 7.8 GiB despite low per-worker RSS, likely due concurrent shared-memmap/page-cache pressure. Only task-owned PIDs `20205,20206,20207,20208,20203,18088` were explicitly SIGTERM'd; f0/f1/f2/f3 checkpoints at 24k/22k/20k/22k and their hashes are retained in `outputs/iclr27_phase88/audit/h2_cpu_parallel_resource_stop.json`. No `.done` or formal validation was claimed. External long-running jobs were observed but not touched.
- After MemFree recovered, the one-worker CPU resume began from f0 step24k, but MemFree again fell to about 19.9 GiB before the first resume checkpoint. Only task-owned PIDs `24805,24803,17486` were stopped; no new checkpoint or metric was produced. Evidence is `outputs/iclr27_phase88/audit/h2_cpu_sequential_resource_stop.json`. The host's external memory load therefore blocks both CUDA and CPU formal continuation; partial H2 remains unselected.
- A wider low-frequency wait for MemFree ≥50 GiB was attempted without touching external jobs; it was stopped after the host continued oscillating below the threshold. Phase88 is now machine-readably classified `P88_H2_FORMAL_RESOURCE_BLOCKED_NO_CONTROLLER`; integrity is PASS with no residual Phase88 process, no public/sealed-like outputs, valid symlink and parseable JSON. The next action requires an isolated RAM-safe window or measured out-of-core implementation before H2 formal validation; no controller or sealed evaluation is authorized from partial checkpoints.

## Phase88R equal-budget H2 (2026-09-09)

- Reclassified the historical H2 resource stops without deleting evidence. The old trigger used `MemFree`, which reached 32.6/26.8/8.2/19.9 GiB while `MemAvailable` remained 99.4/99.1/83.3/91.0 GiB in the recorded artifacts. Phase88R uses only `MemAvailable <31.25 GiB` for safety, with three consecutive 30-second samples; `MemFree`, Cached, Buffers, PSS and Anonymous remain diagnostics. Artifact: `outputs/iclr27_phase88/audit/resource_metric_correction.json`.
- Registered `H2_EQUAL_BUDGET`: each fold resumes the frozen C0v2 fix2 step-20k checkpoint and trains only to step 30k (`+10k`, seed 88002, event_tag fix2, support_mode false, known-suppression weight 1.5). The f0 legacy C0 checkpoint lacked sampler/rollout persistence; this is explicitly recorded as a deterministic resume-state repair in the preregistration and metrics, not presented as exact continuation. No old extended H2 checkpoint initialized the formal route.
- The new dynamic bounded supervisor discovered four available GPUs and ran f0--f3 concurrently (GPU0--3 at launch; no external PID was touched). All folds completed at 30k with atomic `.done`, checkpoint and metrics. MemFree fell below 1 GiB at one point while MemAvailable stayed above 70 GiB; no false resource stop, OOM, or external-process event occurred. A two-step resume contract smoke passed (`start_step=20000`, finite loss, new persistence fields).
- The first TRAIN validation supervisor failed only in Python result aggregation because the environment does not support `dict | dict`; all shard records were preserved. The smallest compatibility repair replaced the union with `dict.update`; rerun completed all four TRAIN-disjoint folds. H2 mean exact selection score was `0.2355237032` versus C0 `0.2017035062`, with H2 winning all four folds. H2 was frozen from TRAIN only in `outputs/iclr27_phase88/audit/h2_equal_frozen_selection.json`.
- The one authorized held 76-positive + 76-negative diagnostic then produced aggregate Commit-CT `4/76` (historical full persistent baseline `3/76`, old C0 diagnostic `2/76`), existing precision `.5926`, existing recall `.0440`, negative false merge `.1579`, anonymous false merge `.0395`, known capture `.1184`, open-world false assignment `.1579`, premature `.6053`, unresolved `0`, duplicate births `128`, category coverage `2`, video coverage `3`; all four correct events came from fold0. This is a frozen diagnostic, not a broad/generalized success and not a sealed/public evaluation.
- The deterministic auxiliary `standard_gcd_stream_v1` manifest and evaluator were added for reporting only. Its stream-level aggregate was all accuracy `.1493`, old `.3704`, new `.1386`, H-score `.2017`, NMI `.3053`, ARI `.0204`; per-fold metrics are retained in `outputs/iclr27_phase88/audit/standard_gcd_stream_metrics.json`. These metrics were not used for H2 selection.
### Phase89 supervisor recovery (2026-09-09)
- H3 `h3_router` f0/f1 were intentionally interrupted by the AGENTS RAM floor after saving resumable checkpoints (`step020113`, `step020087`); no training result was treated as complete and no external PID was touched.
- First actionable execution root cause: `scripts/iclr27_phase89/run_h3_multigpu.py` latched `paused=True` permanently after the resource pause and rejected launched units even when an explicit `paused_resource.json` checkpoint existed. This stranded f2/f3 and would have caused an unsafe duplicate/restart decision.
- Minimal repair: treat only explicit paused-resource markers as resumable units, pass `--resume-launched` with the saved checkpoint/step, and clear the transient pause once active workers drain and `MemAvailable` recovers. Existing markers/checkpoints are preserved; no seed, denominator, evaluator, or data boundary changed.
- A dedicated supervisor interruption was performed after f0/f1 had no active workers; it was not a worker kill. Smoke/compile regression passed. Next action: resume f0/f1 and launch untouched f2/f3 through the repaired single bounded supervisor when RAM is safe.
- The resumed four-fold run later hit the same RAM safety mechanism once more (all folds saved valid 23--24k checkpoints), exposing a second recovery edge: worker exit after `SIGUSR1` was not re-queued by the supervisor. No unit reached a false DONE state and no checkpoint was overwritten. Minimal repair cycle 2 now requeues the exact `paused_resource` checkpoint on clean worker exit; route remains bounded and resumable.
- H3/C0 matched training completed at 30k after the repaired resume path. H3 TRAIN selection mean 0.214988 versus C0_REOPT 0.200753, winning 3/4 folds, so H3 was frozen under the preregistered rule. Its reporting-only held replay reached 21/76 Commit-CT across all folds, but negative false assignment 0.4737, premature 0.6053 and 92 duplicate births fail the safety interpretation; no public/Q1/sealed evaluation was run.
- The true held OLD/NEW stream was evaluated once with the H3 frozen selection (all accuracy micro/macro 0.5693/0.5607; held NEW micro/macro 0.2975/0.3055). Physical compatibility was audited but not run because no legal full-sequence physical-to-OCD adapter exists in the frozen interface; no HOTA/MOTA/IDF1 claim is made. Phase89 is scientifically complete as a diagnostic route, not complete MOT+OCD.

## Phase90 protocol-correct H3 reproduction (2026-09-10)

- Registered Phase90 from source HEAD `04013a26ea5f99d98474f3201304a17aa02208dd`, with frozen Phase89 outputs, support disabled, seed `88002`, base step 20k, endpoint 30k, and TRAIN-only selection. The new namespace is on `/data2/usr_for_deadline/trackocd_phase90/project_outputs` and `outputs/iclr27_phase90` is a symlink; public/DEV+/Q1/sealed labels remain sealed.
- First actionable Phase90 repair was the resource-resume optimizer contract: Phase89's resume command still carried `--reinit-optimizer`, restoring model/RNG but discarding AdamW moments. `scripts/iclr27_phase88/train_controller.py` now records and asserts optimizer/sampler/RNG continuity for `--resume-launched`; initial base launch is `REINIT_ONCE_AT_BASE`, resource pause is `RESTORE_AFTER_PAUSE`. The two-step smoke passed with all six required state fields present.
- H3_REPRO and C0_REOPT_REPRO were launched with one worker per preflight-idle GPU 1/3/7/8. Multiple RAM-floor pauses were handled only through explicit Phase90 paused checkpoints; no external process was terminated. Both matched routes reached four-fold 30k `.done` artifacts, with final checkpoints and resume audit fields preserved. TRAIN-disjoint validation completed for all eight fold routes; held selection has not been run.
- A minimal post-validation selection script error (`sha()` used an unbound loop sentinel) was retained as a failed invocation and repaired without retraining; the next action is the same-path TRAIN-only selection, followed by the preregistered calibrated hierarchy only if H3_REPRO wins.
- To protect the active Phase90 run from `/data1` exhaustion, frozen historical outputs and superseded SimOWT snapshots were moved (not deleted) to `/data2/usr_for_deadline/trackocd_archive/project_cleanup_20260910/` and replaced by symlinks. The exact paths, sizes, file counts, and rationale are in `docs/CLEANUP_20260910_TRACKOCD.md`; no current checkpoint, manifest, report, code, or active input was removed.
