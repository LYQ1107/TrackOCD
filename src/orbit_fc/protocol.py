"""Shared IO and feature/stat builders for ORBIT-FC."""
...
def frozen_known_protos(class_ids):
    """Original-DINO-space known prototypes from train-known track means."""
...
def known_stats(z, P_known, radii, known_ids=None, anchor_best=None,
                ...
                rel=1.0, track_len=8, n_novel=0, include_anchor=True):
    ks = P_known @ z
    if ks.shape[0]:
        best_k = float(ks.max())
...
def novel_stats(z, P_novel, novel_counts, novel_radii, novel_ids=None,
    ...
    if ns.shape[0]:
        ...
        dist_n = 1.0
        support_norm = 0.0
    return [best_n, second_n, margin_n, dist_n, support_norm,
