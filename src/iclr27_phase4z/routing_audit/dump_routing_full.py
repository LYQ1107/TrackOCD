"""Full per-step routing evidence dump on Q1 dev (frozen O1c code path).

Unlike dump_routing.py, this dump does NOT stop at the first model decision:
every causal prefix step of every physical track is recorded, including the
full 48-way known logits, level-1 router logits/probs, TSR state, physical
evidence, and (when aligned) GT role/category. Memory writes mirror the O1c
online semantics (only before the first non-defer decision per track), so the
level-2 evidence is faithful while the dump stays diagnostic.
"""
...
def main():
        ...
        for key in sorted(tracks):
            ...
            for t, r in enumerate(tracks[key]):
                ...
                rec = {
                    "key": list(key), "t": t,
                    "l1": a1,
                    "l1_logits": [float(x) for x in out["l1"][0]],
                    "l1_probs": [float(x) for x in torch.exp(out["l1_lsm"][0])],
                    "top1_p": float(top2[0]), "margin": float(top2[0] - top2[1]),
                    "entropy": float(ent), "energy": float(ene),
                    "kl": [float(x) for x in kl],
                ...
                if first is None and a1 != 2:
                    first = {"t": t, "l1": a1}
                    if a1 == 0:
                        pred = known_list[int(kl.argmax())]
                        first["known_cat"] = pred
                    else:
                        ...
                        if a2 == 0 and memory.size() > 0:
                            ...
                            first["l2"] = "new"
                            first["slot"] = memory.size()
                            memory.create(h, float(r_scalar[id(r)]), {"track": key})
                        else:
                            first["l2"] = "defer"
            ts = {"n_steps": n_steps, "first": first}
            if key in mapping:
                sid = mapping[key]
                ts["sid"] = sid
                ts["gt_cat"] = labels[sid]["ground_truth_category_id"]
                ts["protocol_role"] = labels[sid]["protocol_role"]
            track_summary["_".join(map(str, key))] = ts
    ...
    (out_dir / "tracks.json").write_text(json.dumps(track_summary, indent=2))
