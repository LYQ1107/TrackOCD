"""Collect Phase 6D eval JSONs into a comparison table."""
...
def load_run(name: str, base: Path) -> dict:
    out = {"name": name}
...
def main():
    ...
    ap.add_argument("--out", default="outputs/iclr27_phase6d/eval/comparison.json")
    args = ap.parse_args()
    base = ROOT / "outputs" / "iclr27_phase6d" / "eval"
    runs = [load_run(n, base / n) for n in args.names]
    out = ROOT / args.out
