"""Causal track and memory relation encoders shared by train/runtime."""
...
class CausalTrackEncoder(nn.Module):
    def __init__(self, raw_dim: int = 768, geom_dim: int = 15,
                 hidden: int = 256, residual_scale: float = 0.05) -> None:
        super().__init__()
        ...
        self.residual = nn.Sequential(
            nn.Linear(hidden + raw_dim, 256), nn.GELU(), nn.Linear(256, raw_dim)
        )
    ...
    def forward_sequence(self, raw_seq: Tensor, geom_seq: Tensor, mask: Tensor) -> dict[str, Tensor]:
        ...
        residual = self.residual(torch.cat([
            hidden_seq.reshape(-1, self.hidden), raw_seq.reshape(-1, self.raw_dim)
        ], dim=-1)).reshape(raw_seq.shape[0], raw_seq.shape[1], self.raw_dim)
        semantic_seq = F.normalize(raw_seq + self.residual_scale * torch.tanh(residual), dim=-1)
        return {
            "anchor_seq": raw_seq,
            "semantic_seq": semantic_seq,
            "track_hidden_seq": hidden_seq,
            "anchor": latest_raw,
            "semantic": semantic_seq[batch, latest_index],
            "track_hidden": latest_hidden,
        }
...
class MemoryRelationEncoder(nn.Module):
    def __init__(self, hidden: int = 128) -> None:
        ...
        self.net = nn.Sequential(
            nn.LayerNorm(21), nn.Linear(21, hidden), nn.GELU(),
            nn.Linear(hidden, hidden), nn.GELU(), nn.Linear(hidden, 1),
        )
        self.repr = nn.Sequential(
            nn.LayerNorm(21), nn.Linear(21, hidden), nn.GELU(), nn.Linear(hidden, hidden),
        )

    def forward(self, semantic: Tensor, prototypes: Tensor, prototype_mask: Tensor,
                state_stats: Tensor, previous_evidence: Tensor,
                support_features: Tensor, quality: Tensor) -> dict[str, Tensor]:
        similarity = torch.einsum(
            "bd,bskd->bsk", F.normalize(semantic, dim=-1), F.normalize(prototypes, dim=-1)
        )
        ...
        candidate = torch.cat([
            relation, state_stats, previous_evidence.unsqueeze(-1),
            quality.view(-1, 1, 1).expand(-1, prototypes.shape[1], -1), support,
        ], dim=-1)
        return {
            "state_logits": self.net(candidate).squeeze(-1),
            "state_repr": self.repr(candidate),
        }
