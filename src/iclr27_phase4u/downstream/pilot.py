...
from src.iclr27_phase4u.downstream.runtime import teacher_targets
...
def eval_episode(model, batch, cfg, known_cat_index, known_list, memory, use_hierarchy):
    ...
    slot_cat = []
    n_occ = batch["feats"].shape[0] if batch["feats"].ndim == 3 else batch["feats"].shape[1]
    for i in range(n_occ):
        ...
        if hasattr(model, "begin_occurrence"):
            q_occ = None
            if batch.get("qphys") is not None:
                q_occ = batch["qphys"][i].to(h.device)
            model.begin_occurrence(batch["feats"][i], q_occ)
        first_commit = None
        for t in range(n):
...
def main():
    ...
    if args.tsr_checkpoint:
        ...
        model = HierarchicalTSRCore(
            ...
            use_qphys=args.use_qphys).to(args.device)
