...
from src.iclr27_phase4u.downstream.runtime import run_episode as run_episode_hier
...
def main():
    ...
    ap.add_argument("--tsr-checkpoint", default=None)
    ap.add_argument("--unfreeze-rep", action="store_true")
    ap.add_argument("--rep-lr", type=float, default=1e-4)
    args = ap.parse_args()
    ...
    if args.tsr_checkpoint:
        rep = TSR(arch="gru").to(args.device)
        ck = torch.load(ROOT / args.tsr_checkpoint, map_location=args.device)
        rep.load_state_dict(ck["model"])
        rep.eval()
        tsr_protos = build_tsr_known_protos(rep, args.device)
        model = HierarchicalTSRCore(
            rep, tsr_protos, use_defer=args.use_defer,
            use_qphys=args.use_qphys,
            freeze_rep=not args.unfreeze_rep).to(args.device)
        model.load_t3_init(str(ROOT / "outputs/iclr27_phase4t/t3/checkpoint.pth"),
                           args.device)
    elif args.use_hierarchy:
    ...
    if args.tsr_checkpoint and args.unfreeze_rep:
        rep_params = [p for n, p in model.named_parameters() if n.startswith("rep.")]
        head_params = [p for n, p in model.named_parameters() if not n.startswith("rep.")]
        opt = torch.optim.AdamW([
            {"params": rep_params, "lr": args.rep_lr},
