...
def n_occurrences(batch: dict) -> int:
    """Phase 4T harness bug fix: episodes are (MAX_OCC, max_len, 768) after
    per-sample extraction; the original loop used shape[1] (=max_len) and
    silently processed only the first 8 occurrences. Collated batches have an
    explicit batch dim and are handled separately."""
    if batch["feats"].ndim == 3:
        return batch["feats"].shape[0]
    return batch["feats"].shape[1]
...
def run_episode(model, batch, cfg, known_cat_index, known_list, memory,
    ...
    for i in range(n_occ):
        ...
        if hasattr(model, "begin_occurrence"):
            q_occ = None
            if q_cpu is not None:
                q_occ = torch.tensor(q_cpu[i], device=h.device)
    ...
    novel_idx = [i for i in range(n_occ)
